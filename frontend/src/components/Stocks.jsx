...
const Stocks = () => {
    ...
    const fetchData = async () => {
      ...
      const fetchTickerData = async (ticker) => {
        try {
          ...
          newStockData[ticker] = [{ last: '-', change_percentage: '-', timestamp: '-' }]
        }
      }
  ...
  const renderTableRows = () => {
    return COMMON_STOCKS.map(({ ticker, name }) => {
      const changePercentage = stockData[ticker]?.change_percentage || '-'
      const color = getColor(changePercentage)

      return (
        <tr key={ticker}>
          <td>{ticker}</td>
          <td>{name}</td>
          <td>{stockData[ticker]?.last || '-'}</td>
          <td style={color}>{changePercentage}</td>
